...
module.exports = async function handler(req, res) {
  ...
  try {
    const { coins: candidates, errors: scrapeErrors } = await scrapeRoyalMintSitemap();

    if (!candidates.length) {
      return res.status(200).json({ staged: 0, skipped: 0, found: 0, message: 'No new coins found on Royal Mint sitemap', errors: scrapeErrors });
    }
    ...
    if (!newRows.length) {
      return res.status(200).json({ staged: 0, skipped: candidates.length, found: candidates.length, message: 'All found coins already in sheet or inbox', errors: scrapeErrors });
    }
