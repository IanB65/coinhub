...
async function handleSpinkSync(body, apiKey, sheetId) {
...
module.exports = async function handler(req, res) {
  ...
  // Route to spink sync if action specified
  if (body.action === 'spink') {
    if (!sheetId) return res.status(500).json({ error: 'GOOGLE_SHEET_ID not configured' });
    try {
      return res.status(200).json(await handleSpinkSync(body, apiKey, sheetId));
    } catch (e) {
